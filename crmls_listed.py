...
FIELDNAMES = ['OriginalListPrice', 'ListingKey', 'CloseDate', 'ClosePrice', 'ListAgentFirstName',
...
MASTER_PATH = os.path.join(DATA_DIR, 'listings.csv')
...
def fetch_month_from_api(pull_year, pull_month):
    ...
    params = {
        ...
        '$filter': (
            f"ListingContractDate ge {month_start.isoformat(timespec='milliseconds')}Z "
            f"and ListingContractDate lt {month_end.isoformat(timespec='milliseconds')}Z"
        ),
        '$top': 1000
    }
...
def update_master_csv(month_file, yyyymm):
    ...
    if os.path.exists(MASTER_PATH):
        ...
        print(f"Loaded existing listings.csv with {rows_before} rows.")
    else:
        master_rows = []
        master_fieldnames = FIELDNAMES + ['SourceMonth']
        rows_before = 0
        existing_months = set()
        print("No existing listings.csv found — starting fresh.")
 
    if yyyymm in existing_months:
        print(f"Month {yyyymm} is already present in listings.csv — skipping merge "
    ...
    print(f"listings.csv rows BEFORE this update: {rows_before}")
    print(f"listings.csv rows AFTER this update: {rows_after}")
    ...
    print(f"Saved updated listings.csv to: {MASTER_PATH}")
